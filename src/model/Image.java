  ...
  private void helpTransformOperate(double[][] filter){
    for(int i=0;i<=pixels.length-1;i++){
      for(int j=0;j<=pixels[0].length-1;j++){
        int r= (int) (filter[0][0]*pixels[i][j].getR()+filter[0][1]*pixels[i][j].getG()+filter[0][2]*pixels[i][j].getB());
        int g= (int) (filter[1][0]*pixels[i][j].getR()+filter[1][1]*pixels[i][j].getG()+filter[1][2]*pixels[i][j].getB());
        int b= (int) (filter[2][0]*pixels[i][j].getR()+filter[2][1]*pixels[i][j].getG()+filter[2][2]*pixels[i][j].getB());
        pixels[i][j].setR(r);
        pixels[i][j].setG(g);
        pixels[i][j].setB(b);
      }
      ...
      for(int y=j-(width-1)/2;y<=j+(width-1)/2;y++){
        if(x>=0&&x<=pixels.length-1&&y>=0&&y<=pixels[0].length-1){
          res+=(flag==0?pixels[x][y].getR():flag==1?
                  pixels[x][y].getG():pixels[x][y].getB())*filter[x-i+(len-1)/2][y-j+(width-1)/2];
        }
      }
    }
    return res;
